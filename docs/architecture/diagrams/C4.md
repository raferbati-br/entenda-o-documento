...
5) /api/analyze checks origin/token, fetches image from Redis, and:
   - when ANALYZE_TEXT_ONLY=1 and OCR text is sufficient, calls analyzeDocument with text-only.
   - otherwise uses image (fallback when OCR is incomplete).
6) analyzeDocument calls provider (OpenAI) and postprocess, then records quality counters.
...
10) /metrics reads aggregated counters and renders a simple dashboard.
